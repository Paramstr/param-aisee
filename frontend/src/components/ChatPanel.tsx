...
export function ChatPanel({ lastEvent, className = '' }: ChatPanelProps) {
  ...
  useEffect(() => {
    scrollToBottom();
  }, [messages, currentResponse, rawTranscripts, isProcessing]);
          ...
          case 'raw_transcript':
            ...
            if (rawTranscript && rawTranscript.trim()) {
              ...
              const newRawTranscript: RawTranscript = {
                ...
                hasWakeWord,
                isProcessed: false,
                isInContext: isInContextMode && !hasWakeWord
              };
  ...
  return (
    <div className={`bg-gray-900 rounded-lg p-4 flex flex-col h-full ${className}`}>
      ...
      <div className="text-xs text-gray-400 flex items-center mt-4 pt-3 border-t border-gray-700 flex-shrink-0">
        <div
        ...
        {isTranscribing 
          ...
          : isProcessing 
            ? 'Osmo is thinking...'
            : isInContextMode
              ? 'Accumulating context (2s silence to process)...'
              : 'Ready to listen'}
      </div>
    </div>
  );
